  ...
  // If they were passed then we look for the page's number also the limit
    ...
    if (req.query.sortBy && !req.query.sortMode) {
      res.status(417).json({
        ...
        solution:
          "Chose one of the examples below, note sortMode could be either 'asc' or 'desc'.",
    ...
    // Checks if sortBy is not passed but sortMode is passed to the URI
      ...
      res.status(417).json({
        ...
        solution:
          "Chose one of the examples below, note sortBy could be either 'title' or 'author'.",
        URI1: "https://book-directory-system-api.herokuapp.com/api/books?page=1&limit=5&sortBy=title&sortMode=asc",
        URI2: "https://book-directory-system-api.herokuapp.com/api/books?page=1&limit=5&sortBy=title&sortMode=desc",
        URI3: "https://book-directory-system-api.herokuapp.com/api/books?page=1&limit=5&sortBy=author&sortMode=asc",
        URI4: "https://book-directory-system-api.herokuapp.com/api/books?page=1&limit=5&sortBy=author&sortMode=desc",
    ...
    // Checks if both sortBy and sortMode were passed to the URI
      ...
      if (!(sortByValue == "author" || sortByValue == "title")) {
        res.status(417).json({
          ...
          solution:
            "Chose one of the examples below, note sortBy's value could be either 'author' or 'title' and sortMode's value could be either 'asc' or 'desc'.",
          URI1: "https://book-directory-system-api.herokuapp.com/api/books?page=1&limit=5&sortBy=title&sortMode=asc",
          URI2: "https://book-directory-system-api.herokuapp.com/api/books?page=1&limit=5&sortBy=title&sortMode=desc",
          URI3: "https://book-directory-system-api.herokuapp.com/api/books?page=1&limit=5&sortBy=author&sortMode=asc",
          URI4: "https://book-directory-system-api.herokuapp.com/api/books?page=1&limit=5&sortBy=author&sortMode=desc",
      ...
      // Checks if the sortMode's value that was passed to the URI is needed either in ascending or descending order (with value 'asc' or 'desc')
        ...
        res.status(417).json({
          ...
          solution:
            "Chose one of the examples below, note sortMode's value could be either 'asc' or 'desc'.",
          URI1: "https://book-directory-system-api.herokuapp.com/api/books?page=1&limit=5&sortBy=title&sortMode=asc",
    ...
    // Checks if both sortBy and sortMode were never passed to the URI
      ...
      Book.find()
        ...
        .then((results) => {
          res.status(200).json({
            URI: `https://book-directory-system-api.herokuapp.com/api/books?page=${
...
app.post("/api/books_with_file", (req, res) => {
  ...
  cloudinary.uploader.upload(
    req.body.cover_image,
    { tags: "Uploaded As File", upload_preset: "uploaded_as_file" },
    (cloudErr, Cloudimage) => {
      if (cloudErr) {
        res.status(500).json({
      ...
      if (Cloudimage) {
        ...
        let newBook = new Book({
          title: req.body.title,
          description: req.body.description,
          cover_image: generatedCloudinaryImageId,
          cover_image_url: generatedCloudinaryImageUrl,
          author: req.body.author,
        });
        ...
        newBook
          ...
          .catch((err) => {
            res
              .status(500)
              .send(`We encountered an issue uploading book, try again.`);
          });
      }
    }
  );
